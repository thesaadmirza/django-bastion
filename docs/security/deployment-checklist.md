...
- [ ] PostgreSQL, SQLite, MySQL or MariaDB. Those four are what the suite runs
      against on every push; Oracle is unsupported. The uniqueness constraints
      are enforced on all four, confirmed by introspection rather than assumed.
- [ ] On MySQL or MariaDB, you are running a version of this package that
      retries a deadlocked audit append. Without it, InnoDB gap locks on the
      chain head drop audit records under concurrent logins, and chain
      verification still reports the log as clean.
